...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-surface-500 mt-0.5">Toggle between light and dark themes</p>
          </div>
          <button
            onClick={toggle}
      ...
      {/* Career Target */}
      <Card>
        <h3 className="text-lg font-semibold text-surface-900 dark:text-white mb-4">New Career Target</h3>
        <form onSubmit={handleTargetCreate} className="space-y-4">
          <Input
            label="Role Title"
            value={targetForm.role_title}
            onChange={(e) => setTargetForm((p) => ({ ...p, role_title: e.target.value }))}
            placeholder="e.g., Senior Full-Stack Engineer"
            required
          />
          <Input
            label="Required Skills"
            value={targetForm.required_skills}
            onChange={(e) => setTargetForm((p) => ({ ...p, required_skills: e.target.value }))}
          ...
          <Input
            ...
            value={targetForm.preferred_skills}
            onChange={(e) => setTargetForm((p) => ({ ...p, preferred_skills: e.target.value }))}
            placeholder="Docker, Kubernetes, AWS"
            hint="Comma-separated list"
          />
          <Button type="submit" variant="primary" isLoading={savingTarget}>
            Create Target
          </Button>
        </form>
      </Card>
      ...
      {/* Integrations */}
      <Card>
        <h3 className="text-lg font-semibold text-surface-900 dark:text-white mb-4">Integrations</h3>
        <div className="flex items-center justify-between p-4 rounded-lg bg-surface-50 dark:bg-surface-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-surface-900 dark:text-white">GitHub</p>
              <p className="text-xs text-surface-500">Connected via OAuth</p>
            </div>
          </div>
          <a
